...
"Maintainer:  David O'Trakoun <me@davidosomething.com>
...
if exists('g:loaded_syntastic_handlebars_hbstidy_checker')
    finish
endif
let g:loaded_syntastic_handlebars_hbstidy_checker = 1
...
if !exists('g:syntastic_handlebars_hbstidy_ignore_errors')
    let g:syntastic_handlebars_hbstidy_ignore_errors = []
endif

if !exists('g:syntastic_handlebars_hbstidy_blocklevel_tags')
    let g:syntastic_handlebars_hbstidy_blocklevel_tags = []
endif

if !exists('g:syntastic_handlebars_hbstidy_inline_tags')
    let g:syntastic_handlebars_hbstidy_inline_tags = []
endif

if !exists('g:syntastic_handlebars_hbstidy_empty_tags')
    let g:syntastic_handlebars_hbstidy_empty_tags = []
endif
...
function! SyntaxCheckers_handlebars_hbstidy_IsAvailable() dict
    return executable('tidy')
endfunction

function! SyntaxCheckers_handlebars_hbstidy_GetLocList() dict " {{{1

    " let l:isfragment = 0
    " let l:contents=getline(1, "$")
    " if match(contents, "<head>") ..... @TODO

    " replace "{{ something }}" with HBSSTRING (assuming they're html
    " attributes)
    let l:striptext     = ' | sed ''s/"\([^{]*\){\{2,3\}\([^}]*\)}\{2,3\}\([^"]*\)"/"\1HBSSTRING\3"/g'''

    " replace remaining unquoted {{ something }}
    " with <script src="#"></script>
    let l:stripblocks   = ' | sed ''s/{\{2,3\}\([^}]*\)}\{2,3\}/<script src="#"><\/script>/g'''

    let makeprg = 'cat ' . syntastic#util#shexpand('%')
                \ . l:striptext
                \ . l:stripblocks
                \ . ' | tidy ' . s:Args()
...
function! s:IgnoreError(text) " {{{2
    for item in s:IGNORE_ERRORS + g:syntastic_handlebars_hbstidy_ignore_errors
...
function! s:NewTags(name) " {{{2
    return syntastic#util#shescape(join( s:{toupper(a:name)} + g:syntastic_handlebars_hbstidy_{a:name}, ',' ))
endfunction " }}}2
